...
def ocr_image(orig_image_arr, file_counter):
    ...
    result_value = ''.join(char for char in result_value if char.isdigit())
    result_value = result_value[:1] + '.' + result_value[1:]  # Add a period after the first character
    
    if 160 <= file_counter <= 187 and result_value[-1] == '8':
        result_value = result_value[:-1] + '0'  # Replace last digit with '0' if it's '8'
        
    if 400 <= file_counter <= 435 and result_value[-1] == '8':
        result_value = result_value[:-1] + '0'  # Replace last digit with '0' if it's '8'
        
    if 600 <= file_counter <= 630 and result_value[-1] == '8':
        result_value = result_value[:-1] + '0'  # Replace last digit with '0' if it's '8'
        
    if 780 <= file_counter <= 805 and result_value[-1] == '8':
        result_value = result_value[:-1] + '0'  # Replace last digit with '0' if it's '8'
        
    if file_counter > 960 and result_value[-1] == '8':
        result_value = result_value[:-1] + '0'  # Replace last digit with '0' if it's '8'
...
for filename in sorted(os.listdir(folder_path), key=lambda x: int(x.split('.')[0].zfill(5))):
    if filename.endswith(".jpg"):
        ...
        print(result_value, image_path)


# Write the results to a CSV file
with open('VoltsPerSecond.csv', 'w', newline='') as csvfile:
